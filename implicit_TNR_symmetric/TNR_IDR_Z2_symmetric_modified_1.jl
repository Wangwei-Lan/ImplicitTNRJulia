...
for chimax in 22:2:24
    ...
    for nStep in 1:20
        ...
        wL_single,qnum_new = compute_isometry_Z2(reshape(Env_wL_single,prod(sizeEnv_wL_single[1:2]),prod(sizeEnv_wL_single[3:4])),
        ...
        @tensor Env_wR_single[:] := A[7,8,9,11]*A_single[-2,11,12,-1]*A[2,1,9,5]*A[3,5,12,4]*
                        A[2,1,10,6]*A[3,6,14,4]*A[7,8,10,13]*A_single[-4,13,14,-3]
        ...
        # compute v for truncation
        wR_single,qnum_new = compute_isometry_Z2(reshape(Env_wR_single,prod(sizeEnv_wR_single[1:2]),prod(sizeEnv_wR_single[3:4])),
                            qnumtemp,qnumtemp,chiw_max);
        ...
        @tensor vA_single_L[:] := A_single[8,7,9,12]*A[10,12,13,11]*A[2,1,9,5]*A[3,5,13,4]*A[2,1,16,6]*A[3,6,20,4]*A_single[15,14,16,19]*
                        A[17,19,20,18]*wL_single[7,8,-1]*wL_single[14,15,-2]*wR[11,10,-3]*wR[18,17,-4]
        @tensor vA_single_R[:] := A[8,7,9,12]*A_single[10,12,13,11]*A[2,1,9,5]*A[3,5,13,4]*A[2,1,16,6]*A[3,6,20,4]*A[15,14,16,19]*
                        A_single[17,19,20,18]*wL[7,8,-1]*wL[14,15,-2]*wR_single[11,10,-3]*wR_single[18,17,-4]
        ...
        for j in 1:UpdateLoop
            ...
            #
            #=
            print(" sL_single after update: ")
            f2,f3 = compute_fidelity_impurity_modified(A,A_single,wL,wR,wL_single,wR_single,vL,vR,vL_single,vR_single,sL,sR,sL_single,sR_single,dis;printdetail=true)
            f1 = compute_fidelity_modified(A,wL,wR,vL,vR,sL,sR,dis,printdetail=true)   
            println(f1+f2+f3)
            =#
            ...
            if j%100 ==0
                ...
                print("Iteration : $j ")
                tol = compute_fidelity_modified(A,wL,wR,vL,vR,sL,sR,dis,printdetail=true) / 100  
                compute_fidelity_impurity_modified(A,A_single,wL,wR,wL_single,wR_single,vL,vR,vL_single,vR_single,sL,sR,sL_single,sR_single,dis;printdetail=true)
